...
get '/reset-password/:email/?' do
	...
	if user = User.first(email: params[:email])
		...
		Pony.mail(
  		...
  		body = ''
  		body << "<p>This link takes you to a page where you can enter a temporary password. You should enter a permanent password on your profile page. Remember to Update Account to save. http://#{request.host}/new-password/#{user.pass_reset_key}.</>"
      body << "<p>If you do not want to change your password or you received this email by mistake, just do nothing and your current password will remain active.</p>"
    )
